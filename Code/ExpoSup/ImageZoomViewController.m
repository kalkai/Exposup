...
- (void)viewDidLoad
{
    ...
    if(![parser parseXMLFileAtPath: self.fileName]) {
        UIAlertController* alert = [[UIAlertController alloc] init];
        if([XMLParser getState] == FILE_EMPTY) {
            alert = [Alerts getZoomNotFoundAlert:self.fileName];
        }
...
- (void)showPopup:(CGRect)rect atIndex:(int)idx{
    ...
    popover.view.backgroundColor = [[Config instance] color2];
    ...
    popController.backgroundColor = [[Config instance] color2];
