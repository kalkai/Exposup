...
- (void)viewDidLoad
{
    ...
    if(![parser parseXMLFileAtPath: fileName]){
        ...
        if([XMLParser getState] == FILE_EMPTY) {
            alert = [Alerts getTextNotFoundAlert:fileName];
        }
        ...
        for(int i=0; i<parser.names.count; ++i) {
            ...
            if([type isEqualToString: @"subtitle"]) {
                ...
                if([type isEqualToString: @"slideshow"]) {
                    ...
                    [button addTarget: self action:@selector(toWeb:) forControlEvents: UIControlEventTouchUpInside];
                }
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    else if( [segue.identifier isEqualToString:@"toWeb"] ) {
