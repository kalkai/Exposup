...
- (void)viewDidLoad
{
    ...
    if(![parser parseXMLFileAtPath: standID]) {
        ...
        if([XMLParser getState] == FILE_EMPTY) {
            alert = [Alerts getSlideshowNotFoundAlert:standID];
        }
    ...
    for(NSString *currentImage in parser.images) {
        NSData *data = [NSData  dataWithContentsOfFile: [[LanguageManagement instance] pathForFile: currentImage contentFile: NO]];
        if(data == nil) {
            UIAlertController *alert = [Alerts getImageNotFoundAlert: currentImage];
            [self presentViewController:alert animated:YES completion:nil];
        }
    }
...
- (void)addImage: (NSString *) imagePath toView: (UIView *) view withFrame:(CGRect)frame {
    ...
    if(imageView.image == nil) {
        UIAlertController* alert = [Alerts getImageNotFoundAlert:imagePath];
        [self presentViewController:alert animated:YES completion:nil];
    }
